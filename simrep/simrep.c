...
#define REPLACE_ROUTINE_NAME_STRING L"IoReplaceFileObjectName"  //@ windows 10 이후 ntoskrnl.exe에 정의됨.

#define REPLACE_QUERY_DIRECTORY_FILE_ROUTINE_NAME_STRING "FltQueryDirectoryFile"

#define PSGET_PROCESS_IMAGE_FILE_NAME_ROUTINE_NAME_STRING L"PsGetProcessImageFileName"
...
typedef PCHAR(*PFN_PsGetProcessImageFileName)(PEPROCESS Process);

//@
// 전역 볼륨 매핑 테이블
#define MAX_VOLUME_COUNT 32

typedef struct _VOLUME_MAPPING {
    UNICODE_STRING DosName;     // 예: "C:"
    UNICODE_STRING DeviceName;  // 예: "\Device\HarddiskVolume2"
} VOLUME_MAPPING, * PVOLUME_MAPPING;
...
typedef struct _SIMREP_GLOBAL_DATA {
	...
	//@ pointer to PsGetProcessImageFileName
    // 
	PFN_PsGetProcessImageFileName   PsGetProcessImageFileNameFunction;
...
SimRepChangeName(
    ...
    _Out_ PUNICODE_STRING ChangedPath
    );
...
#pragma alloc_text(PAGE, SimRepChangeName)
...
/**
* Function Name: DebugVolumeName
* Description  : 주어진 볼륨의 NT 경로를 출력합니다.
*                VirtualBox(Windows 11)에서 테스트됨. 10개의 볼륨이 잘 나옴.
* 			     예: \Device\HarddiskVolume3 가 가상 환경과 개발 노트북에서 c:\로 매핑됨을
*                WinObj로 확인함.
* Arguments    : PFLT_VOLUME Volume - 출력할 볼륨 핸들
*/
VOID DebugVolumeName(PFLT_VOLUME Volume, PFLT_FILTER Filter)
{
    NTSTATUS status;
    UNICODE_STRING volumeName;
    ULONG bufferSize = 512; // 충분한 크기 확보
    PWCHAR buffer = NULL;
    ...
    buffer = (PWCHAR)ExAllocatePool2(POOL_FLAG_NON_PAGED, bufferSize, 'rpSM');
    if (buffer == NULL) {
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: Failed to allocate buffer for volume name\n"));
        return;
    }
    ...
    status = FltGetVolumeName(Volume, &volumeName, &bufferSize);
    if (NT_SUCCESS(status)) {
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: Volume NT Path: %wZ\n", &volumeName));

        if (RtlCompareUnicodeString(&volumeName, &targetName, TRUE) == 0) {
            DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: matched. volume name: %wZ, target name: %wZ\n", volumeName, targetName));

            PDEVICE_OBJECT deviceObject = NULL;

            status = FltGetDeviceObject(Volume, &deviceObject);

            if (NT_SUCCESS(status)) {
                /*
                * FltRegisterFilter -> FltStartFiltering -> SimRepInstanceSetup 에서
				* NT_SUCCESS 리턴하여 attach 할 수 있음.
                * But, attach는 자동 호출되는 SimRepInstanceSetup 에서 수행하기로.
                */
                //PFLT_INSTANCE instanceHandle;

                //status = FltAttachVolume(Filter, Volume, NULL, &instanceHandle);

                //if (NT_SUCCESS(status)) {
                //    DbgPrint("[SimRep]: Successfully attached SimRep to HarddiskVolume3 (Volume = %p)\n", Volume);
                //}
                //else {
                //    DbgPrint("[SimRep]: Attach failed: 0x%x\n", status);
                //}
            }
            else {
                DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: FltGetDeviceObject failed: (Status = 0x%x)\n", status));
			}
        }
        //else {
        //    DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: Not matched. volume name: %wZ, target name: %wZ\n", volumeName, targetName));
        //}
    }
    else {
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: FltGetVolumeName failed: (Status = 0x%x)\n", status));
    }
...
NTSTATUS InitializeVolumeMappings(PFLT_FILTER Filter)
{
    NTSTATUS status;
    PFLT_VOLUME volumeList[MAX_VOLUME_COUNT];
    ULONG volumeCount = 0;
    //UNICODE_STRING dosName;

    status = FltEnumerateVolumes(Filter, volumeList, MAX_VOLUME_COUNT, &volumeCount);
    if (!NT_SUCCESS(status)) {
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD,
            ("[SimRep]: Error: fail to FltEnumerateVolumes. volumn count: %lu\n", volumeCount));

        return status;
    }

    for (ULONG i = 0; i < volumeCount; i++) {
        PFLT_VOLUME volume = volumeList[i];
        //PDEVICE_OBJECT deviceObject = NULL;

		DebugVolumeName(volume, Filter);


        // 1. 디바이스 객체 얻기
        //status = FltGetDeviceObject(volume, &deviceObject);
        //if (!NT_SUCCESS(status) || deviceObject == NULL) {
        //    DbgPrint("[SimRep] Volume %u: Failed to get device object\n", i);
        ...
        //if (NT_SUCCESS(status)) {
        //    DbgPrint("[SimRep] Volume %u: %wZ\n", i, &dosName);
        //    ExFreePool(dosName.Buffer);
        //}
        //else {
        //    DbgPrint("[SimRep] Volume %u: (no DOS name)\n", i);
        //}
...
--*/
{
	...
	UNICODE_STRING psgetProceeImageFileName;
    ...
    RtlInitUnicodeString(&psgetProceeImageFileName, PSGET_PROCESS_IMAGE_FILE_NAME_ROUTINE_NAME_STRING);

    Globals.PsGetProcessImageFileNameFunction = MmGetSystemRoutineAddress(&psgetProceeImageFileName);

    if (Globals.PsGetProcessImageFileNameFunction == NULL) {
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD,
            ("[SimRep]: Error: fail to get address of PsGetProcessImageFileName.\n"));
	}
    ...
    DebugTrace( DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: Driver being loaded.\n") );
    ...
    if (!NT_SUCCESS( status )) {
        ...
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD,
            ("[SimRep]: Error: fail to FltRegisterFilter. (Status = 0x%08X)\n", status));
        goto DriverEntryCleanup;
    }

#ifdef DEBUG_PREV_TOO_MANY
    // debug volume names
    status = InitializeVolumeMappings(Globals.hFilter);

    if (!NT_SUCCESS(status)) {
        FltUnregisterFilter(Globals.hFilter);
        goto DriverEntryCleanup;
    }
#endif
    ...
    status = FltStartFiltering( Globals.hFilter );
...
BOOLEAN CheckVolumeName(
    _In_ PFLT_VOLUME Volume,
	_In_ PUNICODE_STRING volNamePattern
)
{
    NTSTATUS status;
    UNICODE_STRING volumeName;
    ...
    buffer = (PWCHAR)ExAllocatePool2(POOL_FLAG_NON_PAGED, bufferSize, 'rpSM');
    if (buffer == NULL) {
        DbgPrint("[SimRep]: CheckVolumeName: Failed to allocate buffer for volume name\n");
        return nRet;
    }
    ...
    if (NT_SUCCESS(status)) {
        //DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: Volume NT Path: %wZ, pattern: %wZ \n", &volumeName, volNamePattern));

        // check if volumeName matches "\Device\HarddiskVolume?" pattern
        if (FsRtlIsNameInExpression(volNamePattern, &volumeName, TRUE, NULL)) {
            DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: Volume NT Path: %wZ will be attached\n", &volumeName));
            nRet = TRUE;
        }
    }
    else {
...
#define MAX_VOLUME_NAME_BYTES 512  // 512 바이트(= 256 WCHAR) 할당
    ...
    ) 
{
    ...
    status = FltGetDiskDeviceObject(FltObjects->Volume, &volDeviceObject);
    if (NT_SUCCESS(status)) {

		RtlInitUnicodeString(&nameInfo, NULL);

        if (volDeviceObject->DeviceType == FILE_DEVICE_DISK) {
            status = IoVolumeDeviceToDosName(volDeviceObject, &nameInfo);
            if (NT_SUCCESS(status)) {
                /**
                * "C:" 나옴
                * "D:" 인 경우 "\\?\Volume{55d39d3c-...}" 와 같이 나옴 (가상 환경이라서 그런 듯)
                */
				DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: drive name: %wZ\n", &nameInfo));

                // "C:"인지 확인
                UNICODE_STRING targetName;
                RtlInitUnicodeString(&targetName, L"C:");
                if (RtlCompareUnicodeString(&nameInfo, &targetName, TRUE) == 0) {
                    bRet = TRUE;
                }

                // 사용 후 메모리 해제
                if (nameInfo.Buffer) {
                    ExFreePool(nameInfo.Buffer);
                }
            }
            else {
			    DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: IoVolumeDeviceToDosName failed: (Status = 0x%x)\n", status));   //<-- failed here (0xc000000d)
		    }

		    ObDereferenceObject(volDeviceObject);
        }
    }
    else {
        DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: FltGetDiskDeviceObject failed: (Status = 0x%x)\n", status));
	}
...
--*/
{
    ...
    if ( FlagOn( Flags, FLTFL_INSTANCE_SETUP_AUTOMATIC_ATTACHMENT ) ) {
        ...
        /*
		* Only attach to NTFS volumes("/Device/HarddiskVolumeX") on automatic attachment.
        * "탑재 지점 없음" volumne도 attach를 한다. 하지만 사용자가 사용할 수 없음으로 성능, 기능에 영향 없음.
		*/
        if (VolumeFilesystemType == FLT_FSTYPE_NTFS) {
            UNICODE_STRING volNamePattern;
            RtlInitUnicodeString(&volNamePattern, L"\\DEVICE\\HARDDISKVOLUME?");

            if (CheckVolumeName(FltObjects->Volume, &volNamePattern) == TRUE) {
                if (IsCDrive(FltObjects) == TRUE) {
                    DebugTrace(DEBUG_TRACE_LOAD_UNLOAD, ("[SimRep]: I got C: drive\n"));
                }
			
                return STATUS_SUCCESS;  // return STATUS_SUCCESS to attach
            }
        }
        ...
        DebugTrace( DEBUG_TRACE_INSTANCES,
                    ("[Simrep]: Instance setup skipped (Volume = %p, Instance = %p, VolumeFilesystemType = %lu)\n",
                    FltObjects->Volume,
                    FltObjects->Instance,
...
Routine Description:
    ...
    ** IRP_MJ_NETWORK_QUERY_OPEN 호출 조건 **
    - **네트워크 파일 시스템(SMB, WebDAV 등)**에서
    - 클라이언트가 파일을 열기 전에 **파일의 메타데이터(크기, 시간 등)**만 빠르게 확인하려고 할 때
    - FastIoQueryOpen 또는 FastIoQueryNetworkOpenInfo와 유사한 동작을 수행
    - 파일을 실제로 열지는 않으며, 빠른 정보 조회 목적으로만 사용됨
...
BOOLEAN CompareProcess(_In_ PFLT_CALLBACK_DATA Cbd,
	_In_ PCHAR targetProcessName
    )
{
    NTSTATUS        status;
    HANDLE          pid;
    PEPROCESS       hProcess = NULL;
    PCHAR           processName = "Unknown";
	BOOLEAN         bRet = FALSE;   

     // get process id
    pid = FltGetRequestorProcessIdEx(Cbd);
    ...
    if (pid != NULL) {
        ...
        if (NT_SUCCESS(status)) {
            //@ get process name
            if (Globals.PsGetProcessImageFileNameFunction) {
                /*
                * File IO를 요청한 thread가 속한 process의 이름을 가져옴
                */
                processName = Globals.PsGetProcessImageFileNameFunction(hProcess);

                if (processName != NULL) {
                    if (_stricmp(processName, targetProcessName) != 0) {
                    }
                    else {
                        //DebugTrace(DEBUG_TRACE_ERROR, ("[SimRep]: SimRepPreCreate => process 'notepad.exe'\n"));
                        bRet = TRUE;
                    }
                }
                //DebugTrace(DEBUG_TRACE_ERROR, ("[SimRep]: SimRepPreCreate => get process name success\n")); //<-- test
            }
            else {
                DebugTrace(DEBUG_TRACE_ERROR, ("[SimRep]: SimRepPreCreate => PsGetProcessImageFileNameFunction is NULL\n"));
            }
        }
    }
...
--*/
{
    ...
    if (FlagOn( Cbd->Iopb->OperationFlags, SL_OPEN_TARGET_DIRECTORY )) {
		...
		//@ move this down because nameInfo is used without initialization
...
#ifdef DEBUG_PREV_TOO_MANY
	//@ 너무 많은 로그 발생시 주석 처리, '/Device/HarddiskVolumeX' 볼륨이 붙을 때마다 발생
    DebugTrace( DEBUG_TRACE_REPARSE_OPERATIONS,
                ("[SimRep]: SimRepPreCreate -> Processing create for file %wZ (Cbd = %p, FileObject = %p)\n",
                 &nameInfo->Name,
                 Cbd,
                 FltObjects->FileObject) );
#endif // DEBUG_PREV_TOO_MANY
    ...
    if (!NT_SUCCESS( status )) {
        ...
        /* 아래와 같은 경로 정보가 nameInfo에 담긴다.
        * \Device\HarddiskVolume3\Windows\Prefetch\VSSVC.EXE-6C8F0C66.pf, VSSVC.EXE-6C8F0C66.pf, pf, \Windows\Prefetch\
        * \Device\HarddiskVolume3\Users\WDKRemoteUser\AppData\Local, Local, (null), \Users\WDKRemoteUser\AppData\
        * \Device\HarddiskVolume3\, (null), (null), \
        * [SimRep] : SimRepPreCreate = > path: \Device\HarddiskVolume3\Windows\System32\NetSetupEngine.dll, NetSetupEngine.dll, dll, \Windows\System32\
        * [SimRep]: SimRepPreCreate => path: \Device\HarddiskVolume3\temp2\b.txt, b.txt, txt, \temp2\
        */
        DebugTrace(DEBUG_TRACE_REPARSE_OPERATIONS, ("[SimRep]: SimRepPreCreate (%wZ) => path: %wZ, %wZ, %wZ, %wZ\n",
            &FltObjects->FileObject->FileName,
            &nameInfo->Name, &nameInfo->FinalComponent, &nameInfo->Extension, &nameInfo->ParentDir));
    }
#endif	// DEBUG_PREV_TOO_MANY
    /*
    * redirect a.txt -> b.txt with hard coded (version 1)
    */
    if (CompareProcess(Cbd, "notepad.exe") == TRUE) {
        /**
        * 'notepad.exe' 가 실행 중에 이 블럭으로 엄청나게 들어 온다. 딱 한번이 아니라.
        */
        UNICODE_STRING sourcePath;
        RtlInitUnicodeString(&sourcePath, L"\\Device\\HarddiskVolume3\\temp\\a.txt");

        /** 
        * 
        * SimRepPreCreate(\temp\a.txt) = > path: \Device\HarddiskVolume3\temp\a.txt, a.txt, txt, \temp\
        */
        //DebugTrace(DEBUG_TRACE_REPARSE_OPERATIONS, ("[SimRep]: SimRepPreCreate (%wZ) => path: %wZ, %wZ, %wZ, %wZ\n",
        //    &FltObjects->FileObject->FileName,
        //    &nameInfo->Name, &nameInfo->FinalComponent, &nameInfo->Extension, &nameInfo->ParentDir));

        if (RtlCompareUnicodeString(&nameInfo->Name, &sourcePath, TRUE) == 0) {
            /**
            * [SimRep]: SimRepPreCreate (\temp\a.txt) => (\temp\a.txt) path: \Device\HarddiskVolume3\temp\a.txt, a.txt, txt, \temp\	
            */
            //DebugTrace(DEBUG_TRACE_REPARSE_OPERATIONS, ("[SimRep]: SimRepPreCreate (%wZ) => (%wZ) path: %wZ, %wZ, %wZ, %wZ\n",
            //    &FltObjects->FileObject->FileName, &Cbd->Iopb->TargetFileObject->FileName,
            //    &nameInfo->Name, &nameInfo->FinalComponent, &nameInfo->Extension, &nameInfo->ParentDir));
            UNICODE_STRING targetPath;
            RtlInitUnicodeString(&targetPath, L"\\Device\\HarddiskVolume3\\temp2\\b.txt");
            ...
            if (NT_SUCCESS(status)) {
                ...
                /**
                * 아래와 같이 로그가 찍히는 것으로 보아 IoReplaceFileObjectName 의 영향을 받는 것으로 보인다.
				* 검색 결과 Cbd->Iopb->TargetFileObject 와 FltObjects->FileObject 는 같은 객체를 가리킨다고 한다.
                * 위의 로그에서는 (\temp\a.txt) 였다.
                * [SimRep]: SimRepPreCreate -> Redirected notepad.exe from \Device\HarddiskVolume3\temp2\b.txt to \Device\HarddiskVolume3\temp2\b.txt	
                */
                //DebugTrace(DEBUG_TRACE_REPARSED_OPERATIONS,	("[SimRep]: SimRepPreCreate -> Redirected notepad.exe from %wZ to %wZ\n",
                //        &FltObjects->FileObject->FileName, &targetPath));
    ...
    /* end of hard coded version 1 */

    //
    //  Change the path from the old mapping to new mapping if the query overlaps
    //  the mapping path. Note: if the create is case sensitive this comparison
    //  must be as well.
    //

    status = SimRepChangeName( nameInfo,
                              &Globals.Mapping.OldName,
                              &Globals.Mapping.NewName,
...
)
{
    ...
    DebugTrace(DEBUG_TRACE_ALL_IO,
        ("[SimRep]: SimRepPostCreate -> Exit (Cbd = %p, FileObject = %p)\n",
            Cbd,
            FltObjects->FileObject));
...
#pragma warning( pop )
    ...
    //    destination. To fix this we'll need to Change the FileName buffer
    //    explicitly.
    //
    ...
    if (status == STATUS_NOT_FOUND) {
        ...
        //  a new filename string Changed with the new mapping to send in the
    ...
    if (!NT_SUCCESS( status )) {
        ...
        //
        //  The rename doesn't overlap the mapping at all. No need to Change
        //
    ...
    //
    //  Explicitly set the Changed the name in the set information structure so
...
--*/
{
    ...
    if (match) {
        ...
        RtlInitUnicodeString( ChangedPath, NULL );

        ChangedPath->MaximumLength = (USHORT)length;

        status = SimRepAllocateUnicodeString( ChangedPath );
        ...
        RtlCopyUnicodeString( ChangedPath, &NameInfo->Volume );

        //
        //  Copy the new file name in place of the matching part of the name
        //

        status = RtlAppendUnicodeStringToString( ChangedPath, NewSubPath );
        ...
        RtlCopyMemory( Add2Ptr( ChangedPath->Buffer, NameInfo->Volume.Length + NewSubPath->Length ),
        ...
        ChangedPath->Length = length;

    }

SimRepChangeNameCleanup:
...
--*/
{
    ...
    match = FALSE;
    exactMatch = FALSE;
	fileName.Buffer = Add2Ptr(NameInfo->Name.Buffer, NameInfo->Volume.Length);    // 파일 경로에서 볼륨 경로(\Device\HarddiskVolumeX) 제외
...
//  not Change the names but simply passes the name queries through.
//
...
--*/
{
    ...
    status = FltCreateFile( Globals.hFilter,
                            Instance,
                            &directoryHandle,
...
--*/
{
    ...
    status = FltCreateFileEx2( Globals.hFilter,
                               Instance,
                               &directoryHandle,
                               &directoryFileObject,
                               FILE_LIST_DIRECTORY | SYNCHRONIZE, // DesiredAccess
